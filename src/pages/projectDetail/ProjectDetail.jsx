...
export default function ProjectDetail() {
  ...
  let generateTypeOfTasks = () => {
    if (projectInfo[1]) {
      return projectInfo[1].map((element) => {
        return (
          <div className="col-12 col-md-6 col-xl-3 boardContainer">
  ...
  let generateTaskMember = (assignees) => {
    return assignees.map((member) => {
      return (
        <img key={member.id} className="taskAva" src={member.avatar} alt="" />
      );
    });
  };
            ...
            <div className="taskContainer_Task">
              ...
              <div className="taskContainer_TaskInfo">
                <div className="taskPriority">
                  <p className="taskPriority_Content">
  ...
  return (
    <div className="projectDetail">
      ...
      <div className="projectDetail_Container">
        <h3 className="projectDetail_Title">
          {projectInfo[0].projectName ? projectInfo[0].projectName : ""}
        </h3>
        <div className="projectDetail_Description">
          {projectInfo[0].description
            ? projectInfo[0].description.replace(/<\/?[^>]+(>|$)/g, "")
            : ""}
        </div>
        ...
        <div className="ProjectDetail_Tasks row">{generateTypeOfTasks()}</div>
      </div>
    </div>
  );
}
